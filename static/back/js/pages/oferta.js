...
var Oferta = function() {
    return {
        init: function(codigo, csrf, action, inicio, fin, cantidad) {
            var id = codigo;
            csrftoken = csrf;
            var accion = action;
            ...
            $("#cantidad").kendoNumericTextBox({ value:cantidad, format: "n0", min: 0 });
            ...
            var dataSource = new kendo.data.DataSource({
                transport: {
                    read: {
                            url: "/admin/ajax/listausuariosoferta/" + id,
            ...
            $('#form-oferta').validate({
                ...
                messages: {
                    nombre: {
                        required: 'Por favor, rellene el nombre de la oferta'
                    }
                },
                submitHandler: function(form) {
                    url = "";

                    if (accion == "C") url = "/admin/ajax/nuevaoferta";
                    if (accion == "U") url = "/admin/ajax/editaroferta";
                    if (accion == "D") url = "/admin/ajax/eliminaroferta";
                    ...
                    if (accion != "D") datos = { csrftoken: csrftoken, nombre: $('input[name=nombre]').val(), inicio: $("#inicio").data("kendoDateTimePicker").value(),
                               fin: $("#fin").data("kendoDateTimePicker").value(), id: id, cantidad: $("#cantidad").data("kendoNumericTextBox").value(),
                               descripcion: $("#descripcion").val(), tipo: "N", imagen: imagen };
        ...
        mostrargs: function() {
            ...
            // Ocultamos la sección de usuarios de la oferta
            $("#usuariosoferta").hide();
